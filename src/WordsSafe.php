...
class WordsSafe
{
     ...
     * @throws
     */
    public function load($sensitiveWords = [], $trieTree = self::LOAD_WORDS)
    {
        if (!is_array($sensitiveWords) || empty($sensitiveWords)) {
            throw new \Exception('The loaded data is empty!');
        }
        if ($trieTree === self::LOAD_TRIE_TREE && isset($sensitiveWords['Normal']) && isset($sensitiveWords['LowCase'])) {
